...
class ASRDataset(Dataset):
    def __init__(
        ...
        self.length = int(len(self.fbank_files) * subset_size)
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Transpose to (T x F) so pad_sequence pads along time dimension
        ...
        if self.partition != "test-clean":
            ...
            batch_shifted      = [shifted_transcript for _, shifted_transcript, _ in batch]
            batch_golden       = [golden_transcript for _, _, golden_transcript in batch]
            ...
            transcript_lengths = torch.LongTensor([len(shifted_transcript) for shifted_transcript in batch_shifted])
